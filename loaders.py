    ...
    **INTERNAL FUNCTION**
    ...
    if not custom_class:
        ...
        if not only_val:
            if dataset != 'stl10':
                ...
                train_set = custom_class(root=data_path, split='unlabeled', download=True, 
                                transform=transform_train, **custom_class_args)
        if dataset != 'stl10':
            test_set = custom_class(root=data_path, train=True, download=True, 
                                transform=transform_test, **custom_class_args)
        else:
            test_set = custom_class(root=data_path, split='unlabeled', download=True, 
                                transform=transform_test, **custom_class_args)
    ...
    if not only_val:
        train_loader = DataLoader(train_set, batch_size=batch_size, 
            shuffle=shuffle_train, num_workers=workers, pin_memory=True, drop_last=True)

    test_loader = DataLoader(test_set, batch_size=val_batch_size, 
            shuffle=shuffle_val, num_workers=workers, pin_memory=True, drop_last=True)
