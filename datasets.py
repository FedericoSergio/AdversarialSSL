...
class CIFAR100(DataSet):
    ...
    A dataset with 500 training images and 100 testing images, with the
    following classes:
...
class STL10(DataSet):
    ...
    def __init__(self, data_path='/tmp/', **kwargs):
        ...
        ds_kwargs = {
            'num_classes': 10,
            'mean': ch.tensor([0.4914, 0.4822, 0.4465]),
            'std': ch.tensor([0.2471, 0.2435, 0.2616]),
            'custom_class': datasets.STL10,
            'label_mapping': None,
            'transform_train': ContrastiveLearningViewGenerator(da.SIMCLR_TRANSFORMS(96)),
            'transform_test': ContrastiveLearningViewGenerator(da.SIMCLR_TRANSFORMS(96))
        }
        ds_kwargs = self.override_args(ds_kwargs, kwargs)
